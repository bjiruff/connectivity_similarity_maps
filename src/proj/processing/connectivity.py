...
from proj.processing.data_loader import DATASETS

def get_connectivity_matrix(cell_types, dataset: str):
    ...
    for type_ in cell_types:
        ...
        for id_ in type_ids:
            ...
            upstream_df = cell_connections_df[cell_connections_df["post_root_id"] == id_]
            ...
            downstream_df = cell_connections_df[cell_connections_df["pre_root_id"] == id_]
        ...
        for id_ in type_ids:
            side = id_to_side[id_]
            if side == dataset_side_names["left"]: left_ids.append(id_)
            elif side == dataset_side_names["right"]: right_ids.append(id_)
            else: other_ids.append(id_)
...
def get_labels(mat):
    ...
    for root_id, type_, side in indices:
        labels.append(f"{root_id} / {type_}({side})")
    return labels